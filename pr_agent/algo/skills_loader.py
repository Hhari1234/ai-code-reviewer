...
def _expand_skill_path(raw_path: str) -> str:
    """Expand environment vars and ~ while honoring HOME on Windows tests."""
    candidate = raw_path.strip()
    if not candidate:
        return candidate

    # Windows often sets HOME in CI/test environments even though expanduser()
    # prefers USERPROFILE/HOMEDRIVE. Prefer HOME for explicit home-relative
    # paths such as '~' and '~\skills' before any generic expanduser() handling.
    if candidate == "~":
