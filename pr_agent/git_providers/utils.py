...
def _resolve_extra_config_to_file(source):
    ...
    if scheme in ("", "file"):
        if scheme == "file":
            # file://C:/... and file://localhost/C:/... are valid Windows local-file
            # URLs. A drive-qualified URL must be recognized before urlparse()
            # rewrites it as a netloc, otherwise a valid local path is treated like
            # a UNC host name and gets malformed.
            raw = parsed.path or ""
            netloc = parsed.netloc or ""
            lower_netloc = netloc.lower()

            if lower_netloc.startswith("localhost"):
                netloc = netloc[len("localhost") :]
                lower_netloc = netloc.lower()

            # Handle file:///C:/... and file://localhost/C:/... forms.
            if raw.startswith("/") and re.match(r"^/[A-Za-z]:", raw):
                local_path = raw[1:]
            elif netloc and re.match(r"^[A-Za-z]:", netloc):
                local_path = netloc + raw
            elif netloc and lower_netloc not in ("", "localhost"):
                local_path = f"//{netloc}{raw}"
            else:
                local_path = raw
